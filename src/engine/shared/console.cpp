...
const IConsole::CCommandInfo *CConsole::CCommand::NextCommandInfo() const
{
...
const IConsole::CCommandInfo *CConsole::FirstCommandInfo() const
{
...
void CConsole::PossibleCommands(const char *pStr, bool Temp, FPossibleCallback pfnCallback, void *pUser)
{
	for(CCommand *pCommand = m_pFirstCommand; pCommand; pCommand = pCommand->m_pNext)
	{
		if(pCommand->m_Temp == Temp)
		{
...
void CConsole::ConToggle(IConsole::IResult *pResult, void *pUser)
{
	...
	char aBuf[128] = {0};
	CCommand *pCommand = pConsole->FindCommand(pResult->GetString(0));
	if(pCommand)
	{
...
void CConsole::ConToggleStroke(IConsole::IResult *pResult, void *pUser)
{
	CConsole* pConsole = static_cast<CConsole *>(pUser);
	char aBuf[128] = {0};
	CCommand *pCommand = pConsole->FindCommand(pResult->GetString(1));
	if(pCommand)
	{
		FCommandCallback pfnCallback = pCommand->m_pfnCallback;
...
void CConsole::PossibleCommandsList(bool Temp, void *pUser)
{
	for(CCommand *pCommand = m_pFirstCommand; pCommand; pCommand = pCommand->m_pNext)
	{
		if(pCommand->m_Temp == Temp)
		{
			char Buf[512];
			str_format(Buf,sizeof(Buf),"Command Name: %s Description: %s", pCommand->m_pName, pCommand->m_pHelp);
			Print(OUTPUT_LEVEL_STANDARD, "Console", Buf);
		}
	}
}

void CConsole::ConHelp(IConsole::IResult *pResult, void *pUser)
{
	CConsole* pConsole = static_cast<CConsole *>(pUser);
	pConsole->PossibleCommandsList(false, pUser);
}

void CConsole::ConHelpRcon(IConsole::IResult *pResult, void *pUser)
{
	CConsole* pConsole = static_cast<CConsole *>(pUser);
	pConsole->PossibleCommandsList(true, pUser);
}
...
CConsole::CConsole()
{
	...
	Register("echo", "r", Con_Echo, this, "Echo the text");
	Register("exec", "r", Con_Exec, this, "Execute the specified file");

	Register("help", "", ConHelp, this, "help command for client commands");
	Register("help_rcon", "", ConHelpRcon, this, "help command for server commands");

	Register("toggle", "sii", ConToggle, this, "Toggle config value");
	Register("+toggle", "sii", ConToggleStroke, this, "Toggle config value via keypress");

	// TODO: this should disappear
	#define MACRO_CONFIG_INT(Name,ScriptName,Def,Min,Max,Desc) \
	{ \
		static CIntVariableData Data = { this, &g_Config.m_##Name, Min, Max }; \
		Register(#ScriptName, "?i", IntVariableCommand, &Data, Desc); \
	}

	#define MACRO_CONFIG_STR(Name,ScriptName,Len,Def,Desc) \
	{ \
		static CStrVariableData Data = { this, g_Config.m_##Name, Len }; \
		Register(#ScriptName, "?r", StrVariableCommand, &Data, Desc); \
	}
...
void CConsole::Register(const char *pName, const char *pParams, FCommandCallback pfnFunc, void *pUser, const char *pHelp)
{
...
void CConsole::RegisterTemp(const char *pName, const char *pParams, const char *pHelp)
{
	CCommand *pCommand;
	if(m_pRecycleList)
	{
...
const IConsole::CCommandInfo *CConsole::GetCommandInfo(const char *pName, bool Temp)
{
	for(CCommand *pCommand = m_pFirstCommand; pCommand; pCommand = pCommand->m_pNext)
	{
		if(pCommand->m_Temp == Temp)
